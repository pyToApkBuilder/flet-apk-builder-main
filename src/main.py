...
def get_close(symbol, exchange="NSE", screener="india"):
    try:
        ...
        data = handler.get_indicators(['description','close','change','volume','RSI','ADX','price_52_week_high','EMA200'])
...
def main(page: ft.Page):
    ...
    text_field = ft.TextField(label="Enter Stock Symbol", expand=True,autofocus=False)
    ...
    def add_card(e):
        ...
        if e.control.text == "Add":
            ...
            stocks_data.insert(0,[stock, data[0]['close'], date(), direction])
            save_data(stocks_data)
            text_field.value = ""
            screenupdate()
        elif e.control.text == "Cancel":
            text_field.value = ""
    ...
    button = ft.ElevatedButton("Enter", on_click=add_stock)
    clear_btn = ft.IconButton(icon=ft.Icons.CLEAR_ALL,on_click=lambda e: page.open(dlg_clear_data))

    page.add(ft.Container(expand=True,padding=ft.padding.only(top=15),content=ft.Column([
            ft.Row([error_message,clear_btn],alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
            ft.Row([text_field,button]),
            result_column,
        ], expand=True)
    ))
